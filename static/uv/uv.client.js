(() => {
  ...
  var require_events = __commonJS({
    "node_modules/.pnpm/events@3.3.0/node_modules/events/events.js"(exports, module) {
      ...
      EventEmitter21.prototype.setMaxListeners = function setMaxListeners(n2) {
  ...
  var DocumentHook = class extends import_events.default {
    ...
    overrideDomain() {
      this.ctx.overrideDescriptor(this.docProto, "domain", {
        get: (target, that) => {
          const event = new hook_default({ value: target.call(that) }, target, that);
  ...
  var NodeApi = class extends import_events3.default {
    ...
    overrideParent() {
      this.ctx.overrideDescriptor(this.nodeProto, "parentNode", {
        get: (target, that) => {
          const event = new hook_default({ node: target.call(that) }, target, that);
    ...
    overrideItemMethods() {
      ...
      this.ctx.override(
        ...
        (target, that, args) => {
          ...
          const event = new hook_default({ namespace, localName }, target, that);
    ...
    overrideFunction() {
      this.ctx.override(
        ...
        (target, that, args) => {
          ...
          const event = new hook_default({ script, args: fnArgs }, target, that);
  ...
  var History = class extends import_events10.default {
    ...
    overridePushState() {
      this.ctx.override(this.historyProto, "pushState", (target, that, args) => {
        ...
        const event = new hook_default({ state, title, url }, target, that);
    ...
    emulate(parse, wrap) {
            ...
            switch (key) {
              ...
              case "hash":
                that.emit(
                  "hashchange",
                  emulation.href,
                  val.trim().startsWith("#") ? new URL(val.trim(), emulation.href).href : new URL("#" + val.trim(), emulation.href).href,
                  that
                );
                break;
              default:
      ...
      if ("reload" in this.location) {
        this.ctx.nativeMethods.defineProperty(emulation, "reload", {
          value: this.ctx.wrap(this.location, "reload", (target, that2) => {
            return target.call(that2 === emulation ? this.location : that2);
          }),
          writable: false,
          enumerable: true
        });
      ...
      if ("replace" in this.location) {
        this.ctx.nativeMethods.defineProperty(emulation, "replace", {
          value: this.ctx.wrap(this.location, "assign", (target, that2, args) => {
            if (!args.length || that2 !== emulation) target.call(that2);
            that2 = this.location;
            let [input] = args;
  ...
  var MessageApi = class extends import_events12.default {
    ...
    overridePostMessage() {
      this.ctx.override(this.window, "postMessage", (target, that, args) => {
        ...
        if (event.intercepted) return event.returnValue;
        return this.ctx.worker ? event.target.call(event.that, event.data.message, event.data.transfer) : event.target.call(
          event.that,
          event.data.message,
          event.data.origin,
          event.data.transfer
    ...
    wrapPostMessage(obj, prop, noOrigin = false) {
      return this.ctx.wrap(obj, prop, (target, that, args) => {
        ...
        this.emit("postMessage", event);
        if (event.intercepted) return event.returnValue;
        return noOrigin ? event.target.call(event.that, event.data.message, event.data.transfer) : event.target.call(
          event.that,
          event.data.message,
          event.data.origin,
          event.data.transfer
  ...
  var Workers = class extends import_events14.default {
    ...
    overridePostMessage() {
      this.ctx.override(this.workerProto, "postMessage", (target, that, args) => {
        ...
        const event = new hook_default({ message, transfer }, target, that);
        this.emit("postMessage", event);
        if (event.intercepted) return event.returnValue;
        return event.target.call(
          event.that,
    ...
    emulate(storage, obj = {}) {
      ...
      const proxy = new this.ctx.window.Proxy(obj, {
        ...
        set: (target, prop, value) => {
          ...
          const event = new hook_default({ name: prop, value }, null, storage);
          this.emit("set", event);
          if (event.intercepted) return event.returnValue;
  ...
  var WebSocketApi = class extends import_events19.default {
    ...
    constructor(ctx) {
      ...
      this.socketmap = /* @__PURE__ */ new WeakMap();
    }
    overrideWebSocket(client) {
      ...
      this.ctx.override(
        ...
        (target, that, args) => {
          const ws = this.socketmap.get(that);
          if (args[0] === void 0) args[0] = 1e3;
          if (args[1] === void 0) args[1] = "";
